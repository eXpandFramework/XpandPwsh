function Write-PSCmdLetBegin {
    ...
    process {
        ...
        if ($Cmdlet.MyInvocation.MyCommand.Parameters){
            ...
            $unboundParameters=@($commandParameters|Where-Object{$_ -notin ($CmdLet.MyInvocation.BoundParameters.Keys+@("ProgressAction"))}|Get-Variable -ErrorAction SilentlyContinue|ForEach-Object{
                [PSCustomObject]@{
                    Name = $_.Name
                    Value=$_.Value
                }
