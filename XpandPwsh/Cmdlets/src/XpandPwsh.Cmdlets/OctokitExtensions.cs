...
namespace XpandPwsh.CmdLets{
    public static class OctokitExtensions{
        public static IObservable<Milestone> LastMileStone(this GitHubClient gitHubClient,string organization,string repositoryName) 
            => gitHubClient.Repository.GetForOrg(organization, repositoryName)
        ...
        public static IObservable<Repository> GetForOrg(this IRepositoriesClient repositoriesClient, string organization,string repositoryName) 
            => repositoriesClient.GetAllForOrg(organization).ToObservable()
                .Select(list => list.First(repository => repository.Name == repositoryName));

        public static IObservable<((GitHubCommit commit, Issue[] issues)[] commitIssues, (Repository repo1, Repository repo2)repoTuple)> 
        ...
        public static IObservable<Release> LastRelease(this IRepositoriesClient repositoriesClient, Repository repository,  string millstone) 
            => repositoriesClient.LastTag(repository, tag => tag.Name!=millstone).Select(tag => tag.Name)
        ...
        public static IObservable<RepositoryTag> LastTag(this IRepositoriesClient repositoriesClient, Repository repository,Func<RepositoryTag,bool> tagFilter=null){
            tagFilter ??= (tag => true);
            return repositoriesClient.GetAllTags(repository.Id).ToObservable()
                .Select(tags => tags.Where(tagFilter ))
                .Select(list => list.First()).FirstAsync();
        }

        public static IObservable<Issue[]> LastMilestoneIssues(this IIssuesClient issuesClient, (Repository repo1, Repository repo2) repoTuple,string millstone=null){
            var millstones = issuesClient.Milestone.GetAllForRepository(repoTuple.repo1.Id).ToObservable()
                .Select(list => list.GetMilestone( millstone)).DefaultIfEmpty();
            var lastMilestoneIssues = issuesClient.GetAllForRepository(repoTuple.repo1.Id).ToObservable()
                .CombineLatest(millstones, (issues, milestone) => (issues, milestone))
        ...
        internal static Milestone GetMilestone(this IEnumerable<Milestone> milestones, string millstone=null){
            ...
            return array.FirstOrDefault(_ => millstone == null ? _.Title == version.ToString() : _.Title == millstone);
        }
        ...
        public static IObservable<GitHubCommit> Commits(this GitHubClient appClient, string organization,
            ...
            => appClient.Repository.GetForOrg(organization, repository)
